...
query_prams <- list("api-key" = nty_apikey, query = movie_name)

# Send the HTTP Request to download the data
# Extract the content and convert it from JSON
response <- GET(paste0(base_uri, resource), query = query_prams)
...
# Print out the list.
review <- list(headline = headline, short_summary = short_summary, link = link)
